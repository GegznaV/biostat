                    ...
                    p_adjust_method = p.adjust.methods[1])
{
    ...
    # TODO: įgalinti kitus galimus p reikšmių skaičiavimo metodus
    p <- switch(method,
                "pearson"  = {2 * (1 - SuppDists::pPearson( abs(rez$cor), N = n))},
                "spearman" = {2 * (1 - SuppDists::pSpearman(abs(rez$cor), r = n))},
                # pspearman::spearman.test
                # pspearman::pspearman

                "kendall"  = {2 * (1 - SuppDists::pKendall( abs(rez$cor), N = n))}
    )
